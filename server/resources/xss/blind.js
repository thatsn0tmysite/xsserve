...
!(function (t, e, n) {
	function r(t, e, n, r) {
		return c(t, n, r, e).then(function (a) {
			...
			return v.ready.then(function () {
				...
				var t =
					"view" === e.type ||
					(u !== h.document.body && u !== h.document.documentElement)
						? i(y.canvas, d)
						: y.canvas;
				return (
					e.removeContainer &&
	...
	function c(e, n, r, i) {
		...
		return (
			...
			new Promise(function (e) {
				...
				(s.contentWindow.onload = s.onload =
					function () {
						e(s);
					}),
					n.open(),
					...
					"view" === i.type &&
						s.contentWindow.scrollTo(t.pageXOffset, t.pageYOffset);
			})
		);
	}
	function h(t) {
	...
	function m(e, n) {
		(this.link = null),
			...
			(this.support = n),
			(this.origin =
				t.location.protocol + t.location.hostname + t.location.port);
	}
	function y(t) {
	...
	function w(t) {
		return { args: [t.node.src], method: "url" };
	}
	function b(t) {
		return { args: [t.node], method: "svg" };
	}
	function x(t) {
			...
			this.colorStops.forEach(function (t, e) {
				null === t.stop &&
					this.colorStops.slice(e).some(function (n, r) {
						return null !== n.stop
							? ((t.stop =
									(n.stop - this.colorStops[e - 1].stop) /
										(r + 1) +
				...
				if (!(0 === u && c.indexOf(t) > -1)) {
					switch (t) {
						...
						case ")":
							if (s) break;
							if (1 === u) {
								if (0 === p) return (u = 0), (o += t), void l();
								p--;
							}
							break;
						case ",":
							if (s) break;
							if (0 === u) return void l();
	...
	function M(t) {
		var e = t.offsetParent ? M(t.offsetParent) : { top: 0, left: 0 };
		return {
	...
	function P(t, e, n, r, i) {
		...
		t !== t.ownerDocument.documentElement &&
			...
			e.rectangle(
				...
				e.width,
				e.height,
				new T(t.ownerDocument.documentElement, null).css(
					"backgroundColor"
				)
			),
	...
	function D(t, e, n) {
		var r = t.left,
			...
			v = s - f;
		return {
			topLeftOuter: L(r, i, a, c).topLeft.subdivide(0.5),
			topLeftInner: L(
				r + n[3].width,
				i + n[0].width,
			...
			topRightOuter: L(r + g, i, h, u).topRight.subdivide(0.5),
			topRightInner: L(
				r + Math.min(g, o + n[3].width),
				i + n[0].width,
				g > o + n[3].width ? 0 : h - n[3].width,
				u - n[0].width
			).topRight.subdivide(0.5),
			bottomRightOuter: L(r + y, i + m, p, l).bottomRight.subdivide(0.5),
			bottomRightInner: L(
				r + Math.min(y, o + n[3].width),
				i + Math.min(m, s + n[0].width),
				Math.max(0, p - n[1].width),
				Math.max(0, l - n[2].width)
			).bottomRight.subdivide(0.5),
			bottomLeftOuter: L(r, i + v, d, f).bottomLeft.subdivide(0.5),
			bottomLeftInner: L(
				r + n[3].width,
				i + v,
				Math.max(0, d - n[3].width),
				Math.max(0, f - n[2].width)
		...
		return ["TopLeft", "TopRight", "BottomRight", "BottomLeft"].map(
			function (e) {
				var n = t.css("border" + e + "Radius"),
					r = n.split(" ");
				return r.length <= 1 && (r[1] = r[0]), r.map(oe);
			}
		);
	}
	function Q(t) {
	...
	function ue(r, i) {
		...
		var c =
			...
			"&callback=" +
			o;
		(this.src = r), (this.image = new Image());
		var h = this;
		this.promise = new Promise(function (r, i) {
			(h.image.onload = r),
				(h.image.onerror = i),
	...
	function we(t) {
		f.apply(this, arguments),
			(this.type =
				"linear" === t.args[0] ? this.TYPES.LINEAR : this.TYPES.RADIAL);
	}
	function be(t) {
			...
			!(function () {
					...
					(o =
						...
						r =
							function (n) {
								...
								if (((o._eak_seen = t), e[n])) return e[n];
								if (((e[n] = {}), !t[n]))
									throw new Error(
				...
				n("promise/asap", ["exports"], function (n) {
					...
					var c,
						...
						p = "undefined" != typeof global ? global : this,
						l = [];
					(c =
	...
	(t.html2canvas = function (i, o) {
		(o = o || {}),
			o.logging &&
				((t.html2canvas.logging = !0),
				(t.html2canvas.start = Date.now())),
			(o.async = "undefined" == typeof o.async ? !0 : o.async),
			(o.allowTaint =
				"undefined" == typeof o.allowTaint ? !1 : o.allowTaint),
			(o.removeContainer =
				"undefined" == typeof o.removeContainer
					? !0
					: o.removeContainer);
		var s = (i === n ? [e.documentElement] : i.length ? i : [i])[0];
		return (
			s.setAttribute(Ee, "true"),
			r(s.ownerDocument, o, t.innerWidth, t.innerHeight).then(function (
				t
			) {
				return (
					"function" == typeof o.onrendered &&
						(E(
		...
		(d.prototype.getMetrics = function (t, e) {
			return (
				this.data[t + "-" + e] === n &&
					(this.data[t + "-" + e] = new l(t, e)),
				this.data[t + "-" + e]
			);
		}),
		(f.prototype.TYPES = { LINEAR: 1, RADIAL: 2 }),
		...
		(m.prototype.isSameOrigin = function (t) {
			...
			(n.href = t), (n.href = n.href);
			var r = n.protocol + n.hostname + n.port;
			return r === this.origin;
		}),
		...
		(m.prototype.fetch = function (t) {
			return (
				(this.images = t.reduce(
					ie(this.findBackgroundImage, this),
					this.findImages(t)
				)),
		...
		(T.prototype.parseBackgroundImages = function () {
			return (
				...
				(this.backgroundImages = S(this.css("backgroundImage")))
			);
		}),
		...
		(T.prototype.parseBackgroundSize = function (t, e, n) {
			...
			if (I(o[0])) r = (t.width * parseFloat(o[0])) / 100;
			else {
		...
		(T.prototype.getValue = function () {
			...
			return (
				(t = "SELECT" === this.node.tagName ? C(this.node) : t),
				0 === t.length ? this.node.placeholder || "" : t
			);
		}),
		(T.prototype.MATRIX_PROPERTY = /(matrix)\((.+)\)/),
		(T.prototype.TEXT_SHADOW_PROPERTY =
			/((rgba|rgb)\([^\)]+\)(\s-?\d+px){0,})/g),
		(T.prototype.TEXT_SHADOW_VALUES =
			/(-?\d+px)|(#.+)|(rgb\(.+\))|(rgba\(.+\))/g),
		...
		(P.prototype.getPseudoElements = function (t) {
			...
			if (t.node.nodeType === Node.ELEMENT_NODE) {
				...
				n &&
					(t.node.insertBefore(n[0].node, t.node.firstChild),
					e.push(n)),
					r && (t.node.appendChild(r[0].node), e.push(r)),
					(n || r) &&
						(t.node.className += " " + this.pseudoHideClass);
			}
			return ce(e);
		}),
		(P.prototype.getPseudoElement = function (t, n) {
			...
			if (((s.className = this.pseudoHideClass), o))
				return (s.src = S(i)[0].args[0]), [a];
			var u = e.createTextNode(i);
			return s.appendChild(u), [a, new ye(u, a)];
		}),
		(P.prototype.getChildren = function (t) {
			return ce(
				[].filter.call(t.node.childNodes, Q).map(function (e) {
					var n = [
		...
		(P.prototype.newStackingContext = function (t, e) {
		...
		(P.prototype.isBodyWithTransparentRoot = function (t) {
			return (
				...
				this.renderer.isTransparent(t.parent.css("backgroundColor"))
			);
		}),
			...
			e.concat(i)
				...
				.forEach(function (t) {
					this.renderQueue.push(t),
						Y(t) && (this.parse(t), this.renderQueue.push(new N()));
				}, this);
		}),
		...
		(P.prototype.paintNode = function (t) {
			...
			switch (
				...
				case "svg":
					...
					r
						? this.renderer.renderImage(t, e, n, r)
						: E("Error loading <svg>", t.node);
					break;
				case "IMG":
					var i = this.images.get(t.node.src);
					i
						? this.renderer.renderImage(t, e, n, i)
						: E("Error loading <img>", t.node.src);
					break;
				case "SELECT":
				case "INPUT":
				case "TEXTAREA":
		...
		(P.prototype.paintText = function (t) {
			...
			var e = t.node.data.split(
					!this.options.letterRendering || G(t) ? /(\b| )/ : ""
				),
			...
			this.renderer.font(
				...
				e.map(this.parseTextBounds(t), this).forEach(function (n, o) {
					n &&
						(this.renderer.text(e[o], n.left, n.bottom),
						this.renderTextDecoration(
							t.parent,
							n,
							this.fontMetrics.getMetrics(i, r)
						));
				}, this);
		}),
	...
	(pe.prototype.renderImage = function (t, e, n, r) {
				...
				switch (r.method) {
					case "url":
						...
						s
							...
							: E("Error loading background-image", r.args[0]);
						break;
		...
		(xe.prototype.setVariable = function (t, e) {
			return (
				this.variables[t] !== e &&
		...
		(xe.prototype.backgroundRepeatShape = function (
			t,
			e,
			n,
			r,
...
// Prevent failure in case the browser refuses to give us any of the probe data.
try {
	results["URI"] = never_null(location.toString());
} catch (e) {
	results["URI"] = "";
}
try {
	results["Cookies"] = never_null(document.cookie);
} catch (e) {
	results["Cookies"] = "";
}
try {
	results["Referrer"] = never_null(document.referrer);
} catch (e) {
	results["Referrer"] = "";
}
try {
	results["UserAgent"] = never_null(navigator.userAgent);
} catch (e) {
	results["UserAgent"] = "";
}
try {
	results["BrowserDate"] = never_null(new Date().getTime().toString());
} catch (e) {
	results["BrowserDate"] = "";
}
try {
...
try {
	results["Origin"] = never_null(location.origin);
} catch (e) {
	results["Origin"] = "";
}
try {
...
try {
	results["Host"] = never_null(document.domain);
} catch (e) {
	results["Host"] = "";
}
...
function hook_load_if_not_ready() {
	try {
		try {
			results["DOM"] = never_null(document.documentElement.outerHTML);
		} catch (e) {
			results["DOM"] = "";
		}
